...
typedef struct cg_parameters {
    int NA1K; //150000
    int NONZER; //15
    int NITER; //75
    double SHIFT; //110.0
    double RCOND; //1.0e-1
    long NZ; //(NA*(NONZER+1)*(NONZER+1))
    long NAZ; //(NA*(NONZER+1))
...
cg_parameters_t buildCGParameters(int na1k /* = [1:9000] */, int nonzer /* = [7:26] */, int niter /* = [10:100] */, double shift /* = [10.0:1500.0] */) {

  printf("building CG Parameters with na1k = %d nonzer = %d niter = %d shift = %e\n", na1k, nonzer, niter, shift);

  const int na = na1k * 1000L;
  const double rcond = 1.0e-1; //keep fixed for now
  const long nz = (na*(nonzer+1)*(nonzer+1));
  const long naz = (na*(nonzer+1));


  cg_parameters_t obj = { .NA1K = na1k, .NONZER = nonzer, .NITER = niter, .SHIFT = shift, .RCOND = rcond, .NZ = nz, .NAZ = naz,
...
void freeCGParameters(cg_parameters_t parameters) {
  free(parameters.colidx);
  free(parameters.rowstr);
  free(parameters.iv);
  free(parameters.arow);
  free(parameters.acol);

  free(parameters.aelt);
  free(parameters.a);
  free(parameters.x);
  free(parameters.z);
  free(parameters.p);
...
//---------------------------------------------------------------------
void conj_grad(int naa, int lastcol, int firstcol, int lastrow, int firstrow, int colidx[],
                      int rowstr[],
                      double x[],
                      double z[],
                      double a[],
...
void makea(int n,
                  ...
                  int lastcol,
                  int arow[],
                  int* acol, //second layer: NONZER+1
                  double* aelt, //second layer: NONZER+1
                  int iv[],
int NONZER,
double RCOND,
double SHIFT);
...
/* common / partit_size / */
  ...
  for (int it = 1; it <= 1; it++) {
    ...
    //---------------------------------------------------------------------
    conj_grad(naa, lastcol, firstcol, lastrow, firstrow, colidx, rowstr, x, z, a, p, q, r, &rnorm);
...
int conjugateGradientPacked(cg_parameters_t parameters)
{
...
)
{
  ...
  for (iouter = 0; iouter < n; iouter++) {
    ...
    for (ivelt = 0; ivelt < nzv; ivelt++) {
      const int idx = iouter*(NONZER+1) + ivelt;
      acol[idx] = ivc[ivelt] - 1;
      aelt[idx] = vc[ivelt];
    }
  }
                   ...
                   double shift)
{
  ...
  for (i = 0; i < n; i++) {
    for (nza = 0; nza < arow[i]; nza++) {
      const int index = (i*(nonzer+1)) + nza;
      j = acol[index] + 1;
  ...
  for (i = 0; i < n; i++) {
    for (nza = 0; nza < arow[i]; nza++) {
      ...
      scale = size * aelt[index];
      for (nzrow = 0; nzrow < arow[i]; nzrow++) {
        const int index2 = (i*(nonzer+1)) + nzrow;
        jcol = acol[index2];
        va = aelt[index2] * scale;
